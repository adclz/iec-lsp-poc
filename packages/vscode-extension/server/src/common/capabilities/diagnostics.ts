...
import { BinderResult, create } from "../symbol-table/binder";
...
import { ExpressionScope } from "../symbol-table/items/scopes/expression";
import { LazySymbol, LazySymbolKind } from "../symbol-table/items/symbols/lazy";
import { referenceMulipleScope } from "../symbol-table/items/scopes/referenceMuliple";
import { QueryCapture, SyntaxNode, Tree } from "web-tree-sitter";


const documentProvider = (singleTons: SingleTons) => {
    let {
        parser,
        queries,
        documents,
    ...
    const openDocument = async (params: DidOpenTextDocumentParams): Promise<void> => {
        ...
        // Init ranges
        const ranges = new IntervalTree<Item>()
...
export const createSymbolTable = (captures: QueryCapture[], ranges: IntervalTree<any>, uri: string): 
{ rootNodes: Node[], lazyItems: Item[], typeChecks: NeedTypeCheck[], diagnostics: Diagnostic[] | null } => {
...
const getErrorNodes = (node: SyntaxNode) => {
...
export const setComments = (ranges: IntervalTree, uri: string, tree: Tree, queries: SingleTons["queries"]) => {
